        ...
        try:
            ...
            for doc in docs:
                ...
                try:
                    ...
                    print(f"⚠️ Date parsing error for transaction: {e}")
                    dt = now
            ...
            print(f"✅ Financials fetched for {user_id}: {result}")
            return result
        except Exception as e:
            print(f"❌ Error fetching user financials: {e}")
            return {
                'income': 0.0,
                'expenses': 0.0,
                'savings_balance': 0.0,
...
class UpdateGoalTool(BaseTool):
    ...
    def _run(self, user_id: str, goal_id: str, updates: dict) -> dict:
        try:
            ...
            print(f"✅ Goal updated {goal_id} for user {user_id}: {updates}")
            return {"success": True}
        except Exception as e:
...
class CreateAlertTool(BaseTool):
    ...
    def _run(self, user_id: str, message: str, email_content: str, risk_level: str) -> dict:
        try:
            ...
            print(f"✅ Goal alert created for user {user_id}")
            return {"success": True}
        except Exception as e:
...
async def analyze_goal(user_id: str, goal_data: Dict[str, Any], context: str = "create") -> Dict[str, Any]:
    ...
    for attempt in range(max_retries):
        try:
            print(f"🎯 Starting unified goal analysis ({context}) for user {user_id} goal {goal_data.get('title')} [Attempt {attempt + 1}/{max_retries}]")
            ...
            print(f"✅ Comprehensive analysis complete ({context}): feasible={analysis_result['feasible']}, risk={analysis_result['risk_level']}")
            ...
            if '503' in error_str or 'overloaded' in error_str.lower():
                print(f"⚠️ API overloaded (503) on attempt {attempt + 1}/{max_retries}")
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"🔄 Rotated to API key {current_api_key_index + 1}, retrying in {retry_delay}s...")
            ...
            print(f"❌ Error in unified goal analysis ({context}): {e}")
            return {
                'feasible': False,
